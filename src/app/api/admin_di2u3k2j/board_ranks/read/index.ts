...
const signBadge = (r: board_rank): board_rank => ({
  ...r,
  badge_image: r.badge_image ? signStoredCloudFrontUrl(r.badge_image) : null,
});

export interface BoardRanksListResponse {
  ranks: board_rank[];
  pagination: PaginationInfo;
}

export interface BoardRanksReadProps {
  page: number;
  pageSize: number;
  order?: "asc" | "desc";
  search?: string;
...
async function getRanksWithPagination(
  ...
  // 정렬 순서
  let orderBy: any = { rank_level: Prisma.SortOrder.desc };
