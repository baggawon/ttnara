...
export interface RankBadgeDeleteProps {
  id: number;
}

export const POST = async (json: RankBadgeDeleteProps) => {
  try {
    ...
    const badge = await handleConnect(async (prisma) => {
      return await prisma.$transaction(async (tx) => {
        const row = await tx.rank_badge_image.findUnique({ where: { id } });
        if (!row) return null;
    ...
    try {
      ...
      console.error("rank badge S3 deletion error:", deleteError);
    }
