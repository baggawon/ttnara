...
import type { BoardRankBadgeAssignConflict } from "@/app/api/admin_di2u3k2j/board_rank_badges/assign";

export const BOARD_RANK_BADGE_MAX_BYTES = 2 * 1024 * 1024;

export const ALLOWED_BOARD_RANK_BADGE_MIME_TYPES = new Set<string>([
  "image/png",
  "image/jpeg",
  "image/jpg",
  "image/webp",
...
export interface BoardRankBadgeUploadResult {
  ok: boolean;
  id?: number;
  name?: string | null;
  url?: string;
  ...
  conflict?: BoardRankBadgeAssignConflict[];
  description?: string;
}
...
export const POST = async (formData: FormData) => {
  try {
    ...
    if (file.size > BOARD_RANK_BADGE_MAX_BYTES) {
      return {
        result: false,
        message: `File too large (max ${BOARD_RANK_BADGE_MAX_BYTES / 1024 / 1024}MB)`,
      };
    }

    if (!ALLOWED_BOARD_RANK_BADGE_MIME_TYPES.has(file.type)) {
    ...
    if (hasRange) {
      ...
      if (existing && existing.length > 0) {
        ...
        if (uniqueUrls.size > 1) {
          ...
          const data: BoardRankBadgeUploadResult = {
            ok: false,
            conflict: existing,
        ...
        const oldBadge = await handleConnect((prisma) =>
          prisma.board_rank_badge_image.findFirst({
    ...
    const uploaded = await uploadFileToS3(file, "board-rank-badges");
    ...
    const row = await handleConnect(async (prisma) => {
      return await prisma.$transaction(async (tx) => {
        const created = await tx.board_rank_badge_image.create({
          data: {
            name,
    ...
    if (hasRange) await appCache.refreshCache(CacheKey.BoardRanks);
    ...
    console.error("board rank badge upload error:", error);
    return {
      result: false,
