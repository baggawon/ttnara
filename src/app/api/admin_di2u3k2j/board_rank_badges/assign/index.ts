...
export interface BoardRankBadgeAssignProps {
  id: number;
  rangeStart: number;
  rangeEnd: number;
}

export interface BoardRankBadgeAssignConflict {
  rank_level: number;
  badge_image: string | null;
}

export interface BoardRankBadgeAssignResponseData {
  ok: boolean;
  conflict?: BoardRankBadgeAssignConflict[];
  description?: string;
}

export const POST = async (json: BoardRankBadgeAssignProps) => {
  try {
    ...
    const result = await handleConnect(async (prisma) => {
      return await prisma.$transaction(async (tx) => {
        const badge = await tx.board_rank_badge_image.findUnique({
          where: { id },
        });
        if (!badge) throw ToastData.unknown;

        const conflictingRanks = await tx.board_rank.findMany({
          where: {
        ...
        if (
          ...
          await tx.profile.updateMany({
            where: {
              current_board_rank_image: badge.aws_cloud_front_url,
              OR: [
                { current_board_rank_level: { lt: rangeStart } },
                { current_board_rank_level: { gt: rangeEnd } },
              ],
            },
        ...
        await tx.profile.updateMany({
          where: {
            current_board_rank_level: { gte: rangeStart, lte: rangeEnd },
          },
          data: { current_board_rank_image: badge.aws_cloud_front_url },
        });
    ...
    if (result.conflict) {
      ...
      const data: BoardRankBadgeAssignResponseData = {
        ok: false,
    ...
    const data: BoardRankBadgeAssignResponseData = { ok: true };
    return { result: true, data };
  } catch (error) {
