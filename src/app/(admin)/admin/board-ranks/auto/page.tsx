...
import { useAdminBoardRanksAutoCreateHook } from "./hook";
...
// this page takes max rank number and max point
// then it creates ranks from 1 to max rank number with point from 0 to max point
// with sensible increment at each rank in between
export default function BoardRanksAutoCreatePage() {
  const {
    methods,
    submit,
    goBack,
    ...
    hasSimulated,
    chartData,
  } = useAdminBoardRanksAutoCreateHook();
                      ...
                      {simulatedRanks.map((rank, index) => (
                        ...
                        >
                          ...
                          <div className="text-sm text-muted-foreground">
                            필요 포인트: {rank.min_point.toLocaleString()}
                          </div>
                        </div>
                      ))}
                    </div>
