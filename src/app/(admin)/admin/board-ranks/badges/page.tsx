...
import { adminBoardRankBadgesGet } from "@/helpers/get";
...
import type {
  BoardRankBadgeListItem,
  BoardRankBadgeListResponse,
} from "@/app/api/admin_di2u3k2j/board_rank_badges/list";
import type {
  BoardRankBadgeAssignProps,
  BoardRankBadgeAssignResponseData,
} from "@/app/api/admin_di2u3k2j/board_rank_badges/assign";
import type { BoardRankBadgeUnassignProps } from "@/app/api/admin_di2u3k2j/board_rank_badges/unassign";
...
export default function BoardRankBadgesPage() {
  ...
  const [assignTarget, setAssignTarget] =
    useState<BoardRankBadgeListItem | null>(null);
  const [rangeStart, setRangeStart] = useState("1");
  const [rangeEnd, setRangeEnd] = useState("1");

  const { data } = useGetQuery<BoardRankBadgeListResponse | null, undefined>(
    { queryKey: [QueryKey.boardRankBadges] },
    adminBoardRankBadgesGet,
    undefined,
    { silent: true }
  );
  ...
  const openAssign = (badge: BoardRankBadgeListItem) => {
  ...
  const submitAssign = async () => {
    ...
    try {
      const { isSuccess, hasMessage, hasData } =
        await postJson<BoardRankBadgeAssignProps>(
          ApiRoute.adminBoardRankBadgesAssign,
          {
            id: assignTarget.id,
            rangeStart: start,
            rangeEnd: end,
      ...
      const payload = hasData as BoardRankBadgeAssignResponseData | false;
      if (!isSuccess) {
        toast({
  ...
  const unassign = async (badge: BoardRankBadgeListItem) => {
    setIsWorking(true);
    try {
      const { isSuccess, hasMessage } =
        await postJson<BoardRankBadgeUnassignProps>(
  ...
  const removeBadge = async (badge: BoardRankBadgeListItem) => {
    setIsWorking(true);
    try {
      const { isSuccess, hasMessage } =
        await postJson<BoardRankBadgeDeleteProps>(
          ApiRoute.adminBoardRankBadgesDelete,
          { id: badge.id }
        );
      toast({
    ...
    <section className="w-full flex flex-col gap-4">
      <div className="flex items-center justify-between gap-2">
        ...
        <Button
          type="button"
          variant="outline"
          onClick={() => router.push(AdminAppRoute.BoardRanks)}
        >
