...
export default function QnaSheet({ isOpen, onClose, qnaId }: QnaSheetProps) {
  ...
  // Populate the form once the existing QnA loads (edit mode). Gated on isOpen
  // so a closed sheet doesn't seed from stale detail data.
  useEffect(() => {
  ...
  }, [isOpen, isEdit, qnaData, reset]);

  // Create mode: start clean each open and pre-select the first active category.
  useEffect(() => {
    if (!isOpen || isEdit) return;
  ...
  }, [isOpen, isEdit, categoriesData, reset, setValue]);
  ...
  return (
    <Sheet open={isOpen} onOpenChange={(o) => !o && onClose()}>
      <SheetContent
        // Match the 이벤트 리스트 (Featured) sheet sizing: ~80–90% of viewport
        // width rather than the default ~25% so the markdown editor has room.
        className="w-full sm:max-w-none sm:w-[95vw] lg:w-[90vw] xl:w-[1200px] overflow-y-auto"
      ...
      >
        <SheetHeader>
          <SheetTitle>{isEdit ? "QnA 수정" : "QnA 추가"}</SheetTitle>
        </SheetHeader>
