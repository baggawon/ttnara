...
- **등급 레벨은 유일**해야 하며, **최소 포인트는 인접 등급과 대소 순서가 맞아야** 합니다 (낮은 레벨보다 크고, 높은 레벨보다 작아야 함). 위반 시 저장 거부.
...
- 입력 필드 + 결과 그리드(9개 초과 시 처음 3·중간 3·마지막 3개) + 라인 차트(X=등급 레벨, Y=필요 포인트)
...
- 입력 필드 + 결과 그리드(9개 초과 시 처음 3·중간 3·마지막 3개) + 결과 차트(X=등급 레벨, Y=필요 거래 횟수)
