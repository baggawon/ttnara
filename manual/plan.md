...
The admin top navigation (`src/components/3_organisms/AdminTopNavigation.tsx`, rendered by `src/app/(admin)/admin/layout.tsx`) groups pages into the following sections. The HTML manual should mirror this grouping.
...
4. **게시판 (Boards)**
   - `/admin/boards/topics` — 개별 설정
   - `/admin/boards/general` — 공통 설정
   - `/admin/board-ranks` — 게시판 등급

5. **거래 게시판 (Trade Board)**
   - `/admin/system-control` — 거래 시스템 제어
   - `/admin/boards/tether` — P2P거래 게시판
   - `/admin/ranks` — 거래 등급

6. **홈 게시판 (Home Board)** — _카드형 홈 게시판이 지정된 경우에만 활성_
   - `/admin/featured` — 이벤트 리스트

7. **채팅 (Chat)**
   - `/admin/chat`

> **변경 이력 메모 (개발 참고):** 과거 매뉴얼에 있던 "로그", "개발팀 안내사항" 비활성 메뉴는 현재 사이드바에서 제거되었습니다. 사용자 등급(`/admin/ranks/create`, `/admin/ranks/[rank]`), QnA(`/admin/support/qna/create`, `/admin/support/qna/[id]`), 게시판 카테고리(`.../categories/[category]`)는 **더 이상 별도 페이지가 아니라 시트(Sheet) 모달**로 동작합니다.
...
- **Sheet/모달 안내**: 생성·수정이 시트 모달로 열리는 페이지는 "이 항목은 목록에서 [생성]/[수정] 버튼을 누르면 우측 시트로 열립니다"라고 명시. 모든 관리자 시트는 **바깥 클릭/ESC로 닫히지 않으며 취소·X 버튼으로만 닫힙니다** (실수 방지).
- **Destructive operations** (`/admin/system-control`, "모든 거래 취소", "거래 기록 리셋", 등급 자동 생성, 토픽/카테고리 삭제) should have a prominent red warning banner.
...
1. 일반 설정 카드 (`GeneralHandleForm`)
2. 시스템 설정 카드 (`SystemHandleForm`)
3. 레벨 설정 카드 (`LevelHandleForm`)
4. 사용자 설정 카드 (`UserHandleForm`)
...
| 이메일         | 이메일    | 비우면 `null`로 저장 (레거시 회원은 이메일이 없을 수 있음) |
| 사용자 레벨    | 숫자      |                                                            |
...
링크 카드(바로가기), QnA 카테고리, QnA 문서를 통합 관리하는 고객지원 본거지입니다. **3개 탭**으로 구성되며, 모든 생성/수정은 **시트(Sheet) 모달**로 열립니다 (별도 페이지 없음).

### ✅ 주요 기능 (탭별)

1. **링크 카드** — FAQ·문의·문서 등 바로가기 카드 관리
2. **QnA 카테고리** — QnA 분류 관리 (이름·순서·활성화)
3. **QnA** — QnA 문서 관리 (질문·답변·카테고리)

### 📋 화면 구성

- 상단 탭 바: `링크 카드 / QnA 카테고리 / QnA` (탭 상태는 URL `?tab=` 로 유지)
- 각 탭에 목록 테이블 + 검색/필터 + [생성] 버튼 → 시트 모달

#### 탭 1 — 링크 카드 (`LinkCardsTab` / `LinkCardsSheet`)

| 항목           | 유형       | 검증                                   |
| -------------- | ---------- | -------------------------------------- |
| 제목           | 텍스트     | 1–200자, 필수                          |
| 설명           | 텍스트영역 | 최대 2000자, 선택                      |
| URL            | 텍스트     | 필수, `http://`·`https://`·`/` 로 시작 |
| 표시 순서      | 숫자       | 0 이상 정수                            |
| 활성화         | 스위치     | 기본 ON                                |
| 새 탭에서 열기 | 스위치     | 기본 ON                                |
| 아이콘 이미지  | 파일       | 선택 (업로드/해제)                     |

- 체크박스 다중 선택 → **선택 삭제** 지원. 검색어/활성화/정렬 필터 제공.
- API: `POST /api/admin_di2u3k2j/support/link-cards/create|update|delete`

#### 탭 2 — QnA 카테고리 (`CategoriesTab` / `CategoriesSheet`)
...
- 카테고리별 소속 QnA 개수 표시.
- **삭제 시 경고**: "카테고리를 삭제하면 소속된 N개의 QnA도 함께 영구 삭제됩니다." → **비활성화 권장**.
- API: `POST /api/admin_di2u3k2j/support/qna-categories/create|update|delete`

#### 탭 3 — QnA (`QnaTab` / `QnaSheet`)

| 항목      | 유형                                | 검증                                                                          |
| --------- | ----------------------------------- | ----------------------------------------------------------------------------- |
| 카테고리  | 드롭다운                            | 필수, **활성 카테고리만** 선택 가능                                           |
| 질문      | 텍스트                              | 필수                                                                          |
| 답변      | 리치 에디터(`SimpleMarkdownEditor`) | 이미지/영상 업로드 지원 (최대 10개, 개당 30MB; jpg·png·gif·webp·mp4·webm·mov) |
| 표시 순서 | 숫자                                | 0 이상 정수                                                                   |
| 활성화    | 스위치                              | 기본 ON                                                                       |

- 체크박스 다중 선택 → **선택 삭제** 지원. 질문 검색/카테고리·활성화 필터 제공.
- API: `POST /api/admin_di2u3k2j/support/qna/create|update|delete`

### ⚠️ 주의사항

- 변경은 사용자에게 즉시 노출됩니다.
- 비활성 카테고리에는 QnA를 새로 배정할 수 없습니다 (드롭다운에 활성만 노출).
- 모든 시트는 바깥 클릭/ESC로 닫히지 않습니다 — 취소/X 로만 닫기.

### 🔌 관련 API

- `GET/POST /api/admin_di2u3k2j/support/link-cards/read|create|update|delete`
- `GET/POST /api/admin_di2u3k2j/support/qna-categories/read|create|update|delete`
- `GET/POST /api/admin_di2u3k2j/support/qna/read|create|update|delete`
...
사용자에게 발송되는 거래성/인증 이메일의 **제목·본문·하단 문구**를 편집하고, **발신(From) 이메일 주소 목록**을 관리합니다. 인증번호(OTP), 비밀번호 재설정 등 시스템 템플릿이 기본 제공됩니다.

### ✅ 주요 기능

- 템플릿 선택 후 **제목 / 본문 / 하단 영역** 개별 편집
- **변수 삽입 버튼**으로 `{{변수}}` 토큰을 커서 위치에 삽입
- **새로고침** 버튼으로 실시간 HTML 미리보기(더미 값 대입) 확인
- 템플릿별 **발신 이메일** 선택 + **문의 이메일** 지정
- (비시스템 템플릿) 활성화 토글
- **발신 이메일 레지스트리** 관리: 추가/수정/삭제, 기본 발신자 지정, 활성화
- **발신 이메일 저장** 버튼으로 레지스트리 일괄 저장

### 📋 화면 구성 (편집 항목)

| 항목        | 유형       | 검증 / 비고                                                       |
| ----------- | ---------- | ----------------------------------------------------------------- |
| 제목        | 텍스트     | 필수(1자 이상). 이메일 제목 헤더로 사용                           |
| 본문        | 텍스트영역 | 필수(1자 이상). 빈 줄 2개 = 문단 분리. `{{authCode}}`는 굵게 렌더 |
| 하단 영역   | 텍스트영역 | 선택. 본문 아래 12px 회색 문구 + 구분선                           |
| 발신 이메일 | 드롭다운   | 등록된 발신자 중 선택 (미선택 시 기본 발신자 사용)                |
| 문의 이메일 | 텍스트     | 선택. `{{contactEmail}}`로 본문/하단에서 mailto 링크로 렌더       |
| 활성화      | 스위치     | 시스템 템플릿은 항상 활성                                         |
...
|---|---|
| `{{siteName}}` | 사이트/브랜드명 |
| `{{authCode}}` | 인증번호 (발송 시 생성, 굵게 렌더) |
| `{{date}}` | 요청 시각 |
| `{{validityMinutes}}` | OTP 유효 시간(분) |
| `{{contactEmail}}` | 문의 이메일 (mailto 링크) |
| `{{siteUrl}}` | 루트 도메인 URL |
| `{{year}}` | 현재 연도 |

### ⚠️ 주의사항

- **시스템 템플릿**(`is_system`)은 **삭제 불가**, 편집만 가능. 최초 진입 시 누락된 시스템 템플릿은 자동 시드됩니다.
- 발신 주소는 **AWS SES에 사전 검증(verified)** 되어 있어야 실제 발송됩니다.
- 발신자 선택 우선순위: ① 템플릿에 지정된 활성 발신자 → ② 활성 + 기본 발신자 → ③ 환경변수 `SES_SENDER_EMAIL`.
- 발신자를 삭제하면 그를 참조하던 템플릿의 `sender_id`는 `null`로 되돌아가 기본 발신자로 발송됩니다.

### 🔌 관련 API

- `POST /api/admin_di2u3k2j/email-templates/read`
- `POST /api/admin_di2u3k2j/email-templates/update`
- `POST /api/admin_di2u3k2j/email-templates/preview`
- `POST /api/admin_di2u3k2j/email-templates/senders/save`
...
공식 보증(거래 안전성을 책임지는 제휴사)을 관리합니다. **2개 탭**으로 구성됩니다: `공식보증업체 목록` / `설정`(보증 배너 + 지역 관리).

### ✅ 주요 기능

- **공식보증업체 목록 탭**: 보증업체 등록 / 수정 / 삭제, 로고·링크·표시 순서·활성화 설정
- **설정 탭 — 보증 배너**: 제목 / 이미지 / 링크 / 활성화 수정
- **설정 탭 — 지역 관리**: 보증 지역 추가/수정/삭제·복구 (소프트 삭제)

### 📋 화면 구성

- **목록 탭**: 로고 / 업체명 / 링크 / 활성화 / 표시 순서 / 작업
- **설정 탭**: 보증 배너 카드(`AdminGuaranteeBannerCard`) + 지역 관리 카드(`AdminGuaranteeRegionsCard`)

### ⚠️ 주의사항

- 공식 보증업체는 계약·검증을 거친 업체여야 합니다.
- 배너 설정은 전역으로 모든 사용자에게 영향을 줍니다.
- 지역은 **소프트 삭제**(`is_active=false`)로, 삭제해도 DB에서 제거되지 않고 복구 가능합니다.

### 🔌 관련 API

- `GET/POST /api/admin_di2u3k2j/guarantee/banner/read|update`
- `GET/POST /api/admin_di2u3k2j/guarantee/read|create|update|delete`
- `GET/POST /api/admin_di2u3k2j/guarantee_region/read|update|delete|restore`

---

# 4. 게시판 (Boards)

## `/admin/boards/topics` — 개별 설정 (토픽 목록)
...
- 변경은 홈에 즉시 반영됩니다.
- 토픽을 **카드형 홈 게시판(fullview)** 으로 지정하면 "홈 게시판" 메뉴(`/admin/featured`)가 활성화됩니다.
...
특정 토픽의 소분류(카테고리)를 관리합니다. 생성/수정은 **시트(Sheet) 모달**(`CategorySheet`)로 열립니다 (별도 `[category]` 페이지 없음).
...
- 목록: 이름 / 설명 / 표시 순서 / 활성화 / 작업
- **시트 입력 항목**: 이름(`validateToipcName`) / 설명(텍스트영역, 빈값은 `null`) / 표시 순서(`validateTopicDisplayOrder`) / 활성화(기본 ON)

### ⚠️ 주의사항

- 카테고리는 단일 상위 토픽에 속합니다.
- **삭제 경고**: "이 카테고리를 삭제하면 하위 게시글, 댓글, 추천까지 모두 영구 삭제되며 복구할 수 없습니다. 비활성화를 권장합니다." → 비활성화/삭제 두 경로 제공.
- **카드형 홈 게시판(fullview)의 카테고리는 외부(Amado) 소스에서 자동 동기화**되므로 이 화면에서 직접 추가·수정·삭제할 수 없습니다 (버튼 비활성 + 안내 배너).

### 🔌 관련 API

- `GET /api/admin_di2u3k2j/topics/read`
- `POST /api/admin_di2u3k2j/topics/categories/update` (생성·수정 공용)
- `POST /api/admin_di2u3k2j/topics/categories/delete`
- `POST /api/admin_di2u3k2j/topics/categories/bulk-create` (홈 게시판 누락 카테고리 자동 생성)

---

## `/admin/boards/general` — 공통 설정 (전역 게시판 설정)
...
**게시판 포인트(`profile.point`)** 에 따라 부여되는 사용자 등급을 관리합니다. 커뮤니티 활동 기여도를 시각화하는 등급 시스템으로, **거래 횟수 기반의 "거래 등급"(`/admin/ranks`)과는 완전히 독립된 별도 시스템**입니다. (DB: `board_rank` vs `rank`)

### ✅ 주요 기능

- 이름/설명으로 등급 검색, 정렬(최신/과거), 필터 초기화
- 단건 등급 **생성**(시트) / **수정**(시트) / **삭제**
- **등급 자동 생성**으로 이동 (`/admin/board-ranks/auto`)
- **배지 이미지 관리**로 이동 (`/admin/board-ranks/badges`)
- 페이지네이션 (기본 10개; 데스크톱 테이블 / 모바일 카드)

### 📋 화면 구성 (테이블 컬럼)

- 등급(`rank_level`) / 이름 / 최소 포인트(`min_point`) / 설명 / 배지 이미지 / 활성화 / 생성·수정일 / 작업(수정·삭제)

**생성/수정 시트 입력 항목**
| 항목 | 유형 | 검증 |
|---|---|---|
| 이름 | 텍스트 | 1–10자, 필수 (`validateRankName`) |
| 등급 | 숫자 | 최소 1, 고유 |
| 최소 포인트 | 숫자 | 0–10,000,000 |
| 설명 | 텍스트영역 | 선택 |
| 활성화 | 스위치 | 기본 ON |
| 배지 이미지 | 파일(수정 시트에만) | PNG/JPG/WebP/SVG, 최대 2MB |

### ⚠️ 주의사항

- **등급은 유일**해야 하며, **최소 포인트는 인접 등급과 대소 순서가 맞아야** 합니다 (낮은 레벨보다 크고, 높은 레벨보다 작아야 함). 위반 시 저장 거부.
- 배지 **해제** 시 그 이미지를 공유하는 **모든 등급**에서 함께 해제됩니다 (확인 다이얼로그).
- 등급 삭제 시 해당 등급 보유 사용자는 현재 포인트 기준으로 재평가되어 적격 등급으로 자동 조정됩니다.

### 🔌 관련 API

- `GET /api/admin_di2u3k2j/board_ranks/read`
- `POST /api/admin_di2u3k2j/board_ranks/create|update|delete`
- `GET /api/admin_di2u3k2j/board_rank_badges/list`
- `POST /api/admin_di2u3k2j/board_rank_badges/upload|unassign`

---

## `/admin/board-ranks/auto` — 게시판 등급 자동 생성

### 🎯 목적

최대 등급 수·최대 포인트·진행 곡선을 지정해 전체 게시판 등급을 자동 생성합니다. 시뮬레이션 후 저장합니다.

### ✅ 주요 기능

- 최대 등급 수(`maxRank`, 기본 100) / 최대 포인트(`maxPoint`, 기본 100000) 입력
- 진행 유형 선택 (선형/오목(convex)/볼록(concave))
- 진행 속도(`progressionRate`, 1.0–5.0, 비선형일 때만) 조정
- **계산** 버튼으로 시뮬레이션 (그리드 + 차트)
- **저장** 버튼으로 확정 (확인 다이얼로그)

### 📋 화면 구성

- 입력 필드 + 결과 그리드(9개 초과 시 처음 3·중간 3·마지막 3개) + 라인 차트(X=등급, Y=필요 포인트)

### ⚠️ 주의사항 (Destructive)

- **저장 시 기존 게시판 등급을 전부 삭제하고 모든 배지 할당을 해제한 뒤 새로 생성합니다.**
- 확인 메시지: "시뮬레이션대로 자동을 진행하시겠습니까? 기존의 등급를 모두 삭제합니다."

### 🔌 관련 API

- `POST /api/admin_di2u3k2j/board_ranks/batch-create`

---

## `/admin/board-ranks/badges` — 게시판 등급 배지 이미지 관리

### 🎯 목적

게시판 등급 배지 이미지를 업로드하고, 특정 등급 범위에 일괄 할당/해제합니다.

### ✅ 주요 기능

- 배지 이미지 업로드 (이름 선택 입력)
- 등급 범위(예: 1~5)로 일괄 할당 / 범위 수정 / 할당 해제 / 삭제
- 전체 배지 목록 보기

### 📋 화면 구성

- **업로드 섹션**: 이름 입력 + 파일 선택 (PNG/JPG/WebP/SVG, 최대 2MB)
- **배지 목록**: 썸네일 / 이름(또는 `배지 #ID`) / 할당 상태("미할당" / "등급 N 할당됨" / "등급 N–M 할당됨") / 범위 할당·수정·해제·삭제 버튼

### ⚠️ 주의사항

- 범위 검증: 시작 ≤ 끝, 시작 ≥ 1. 위반 시 "등급 범위가 올바르지 않습니다" 토스트.
- 다른 배지와 **범위 겹침 불가** — 충돌 시 "다른 배지 이미지가 이미 할당되어 있습니다" + 상세 메시지.
- 배지 삭제 시 할당된 등급의 배지도 함께 해제됩니다.

### 🔌 관련 API

- `GET /api/admin_di2u3k2j/board_rank_badges/list`
- `POST /api/admin_di2u3k2j/board_rank_badges/upload|assign|unassign|delete`
...
## `/admin/ranks` — 거래 등급 (목록)

### 🎯 목적

사용자 **거래 횟수** 기반의 거래 등급(랭크)을 관리합니다. 단건 생성/수정/삭제(시트), 자동 생성, 배지 이미지 관리 진입점을 제공합니다. (게시판 포인트 기반의 "게시판 등급"과는 별개)

### ✅ 주요 기능

- 이름/설명으로 등급 검색 (검색 필드 선택 가능), 정렬(최신/오래된)
- 단건 등급 **생성**(시트) / **수정**(시트) / **삭제**
- **자동 등급 생성**으로 이동 (`/admin/ranks/auto`)
- **배지 이미지 관리**로 이동 (`/admin/ranks/badges`)

### 📋 화면 구성 (테이블 컬럼)

- 이름 / 등급 / 최소 거래 횟수 / 배지 이미지 / 활성화 / 작업 버튼

**생성/수정 시트 입력 항목**
| 항목 | 유형 | 검증 |
|---|---|---|
| 이름 | 텍스트 | 필수 (`validateRankName`) |
| 등급 | 숫자 | 최소 1, 고유 |
| 최소 거래 횟수 | 숫자 | 0–10,000,000, 인접 등급과 순서 검증 |
| 설명 | 텍스트영역 | 선택 |
| 활성화 | 스위치 | 기본 ON |
| 배지 이미지 | 파일(수정 시트에만) | PNG/JPG/WebP/SVG, 최대 2MB |
...
- **등급은 유일해야 합니다** (중복 불가, 보통 1·2·3 순차).
- 상위 등급의 **최소 거래 횟수**는 하위 등급보다 커야 합니다.
- 배지 이미지는 여러 등급이 공유할 수 있어, 해제 시 모든 사용 등급에 영향 (확인 다이얼로그).
...
- `POST /api/admin_di2u3k2j/ranks/create|update|delete|batch-create`
- `GET /api/admin_di2u3k2j/rank_badges/list`
- `POST /api/admin_di2u3k2j/rank_badges/upload|assign|unassign|delete`

---

## `/admin/ranks/auto` — 거래 등급 자동 생성

### 🎯 목적

최대 등급 수, 최대 거래 횟수, 진행 곡선을 지정해 전체 거래 등급을 자동 생성합니다. 시뮬레이션 후 저장합니다.

### ✅ 주요 기능

- 최대 등급 수(`maxRank`) / 최대 거래 횟수(`maxTradeCount`) 입력
- 진행 유형 선택 (선형 / 오목 / 볼록)
- 진행 속도(1.0–5.0, 비선형일 때만) 조정
- **계산** 버튼으로 시뮬레이션 → **저장** 버튼으로 확정 (확인 다이얼로그)

### 📋 화면 구성

- 입력 필드 + 결과 그리드(9개 초과 시 처음 3·중간 3·마지막 3개) + 결과 차트(X=등급, Y=필요 거래 횟수)

### ⚠️ 주의사항 (Destructive)

- **저장 시 기존 거래 등급을 전부 삭제하고 새로 생성합니다.**
- 진행 유형: **선형**(균등 증가) / **오목**(초반 완만·후반 가파름) / **볼록**(초반 가파름·후반 완만).
...
## `/admin/ranks/badges` — 거래 등급 배지 이미지 관리

### 🎯 목적

거래 등급 배지 이미지를 업로드하고, 특정 등급 범위에 일괄 할당/해제합니다.

### ✅ 주요 기능

- 배지 이미지 업로드 (이름 선택 입력)
- 등급 범위로 일괄 할당 / 범위 수정 / 할당 해제 / 삭제
- 전체 배지 목록 보기

### 📋 화면 구성

- **업로드 섹션**: 이름 입력 + 파일 선택 (PNG/JPG/WebP/SVG, 최대 2MB)
- **배지 목록**: 썸네일 / 이름(또는 `배지 #ID`) / 할당 상태 / 범위 할당·수정·해제·삭제 버튼

### ⚠️ 주의사항

- 범위 오류(시작 > 끝, 시작 < 1 등): "범위 설정에 오류가 있습니다" 토스트.
- 다른 배지와 **범위 겹침 불가**.
- 배지 삭제 시 할당된 모든 등급에서 자동 해제.
...
> ⚠️ 이 메뉴는 **토픽 하나를 "카드형 홈 게시판"(special_board_type: `fullview`)으로 지정**해야 활성화됩니다. 지정된 토픽이 없으면 메뉴는 비활성(회색)이며, 진입 시 토픽 목록으로 이동합니다. 지정은 `/admin/boards/topics`에서 합니다.

## `/admin/featured` — 이벤트 리스트 (Featured / Amado 연동)

### 🎯 목적

외부 이벤트 플랫폼 **Amado(playamado.com)** 의 이벤트를 카드형 게시글로 만들어 메인 홈 카드 게시판과 추천 캐러셀에 노출합니다. Amado 이벤트 데이터를 로컬 게시글에 연결·동기화합니다.

### ✅ 주요 기능

- **새로고침**: Amado API에서 최신 이벤트를 수동으로 가져오기 (실패 시 번들 샘플로 폴백)
- **필터**: 상태(전체/진행중/마감·종료), 게시글 작성 여부(전체/미작성/작성됨)
- **게시글 만들기**: 미작성 이벤트로 게시글 생성 (시트)
- **게시글 수정/삭제**: 작성된 이벤트의 게시글 편집/삭제 (시트)
- **운영자 PICK**: 별 아이콘으로 `is_featured` 토글 (홈 캐러셀 노출, 낙관적 UI)
- **누락 카테고리 처리**: Amado 카테고리가 토픽에 없으면 배너/다이얼로그로 자동 생성·관리 유도

### 📋 화면 구성

**이벤트 카드(읽기 전용 표시)**: 카테고리 / 제목 / 마감 시각(`moment_of_truth`, 상대시간) / 거래량(`volume_krw`, 억·만 단위) / 상태(진행중·마감·종료) / 만료(archived) 배지 / PICK 별

**게시글 작성/수정 시트 입력 항목**
| 항목 | 유형 | 검증 |
|---|---|---|
| 제목 | 텍스트 | 2–255자, 필수 (생성 시 이벤트 제목 자동 입력) |
| 설명 | 텍스트영역 | 최대 300자, 카드에 표시 |
| 카테고리 | 드롭다운 | 필수 (토픽에 카테고리 없으면 비활성) |
| 본문 | 마크다운/HTML 에디터 | 선택 (토픽 설정 시 이미지 업로드 지원) |
| 썸네일 | 미디어 | 선택 (토픽이 썸네일 허용 시) |
| 액션 URL 1 / 라벨 | URL / 텍스트 | 필수 (기본값=Amado 상세 URL), 라벨 ≤ 50자 |
| 액션 URL 2 / 라벨 | URL / 텍스트 | 선택 |
| 운영자 PICK | 스위치 | 기본 OFF |

### ⚠️ 주의사항

- 지정된 카드형 홈 토픽이 없으면 페이지 사용 불가 (토픽 목록으로 리다이렉트).
- **Amado API 실패 시** 번들 샘플로 폴백 + "실시간 데이터 없음(샘플 표시 중)" 경고. 약 1시간 캐시.
- **이벤트당 게시글 1개** (`@@unique([topic_id, amado_event_id])`) — 중복 작성 시 "이미 이 이벤트로 작성된 게시글이 있습니다".
- 업스트림에서 사라진 이벤트의 게시글은 **자동 삭제되지 않고 "만료" 배지**로 표시되며 계속 편집 가능.
- **삭제는 게시글 + 첨부 이미지를 영구 삭제** (확인 다이얼로그).
- **누락 카테고리 다이얼로그(`MissingCategoriesDialog`)**: Amado 이벤트가 참조하는 카테고리가 토픽에 없을 때 표시 — "나중에" / "카테고리 관리로 이동" / "자동 생성(N)". 자동 생성은 이름만 만들고 설명·순서는 카테고리 페이지에서 별도 편집.

### 🔌 관련 API

- `POST /api/admin_di2u3k2j/amado/events/read`
- `POST /api/admin_di2u3k2j/featured/post/create|read|update|delete`
- `POST /api/admin_di2u3k2j/featured/post/toggle-featured`
- `POST /api/admin_di2u3k2j/topics/categories/bulk-create`
...
- **신고에는 별도 처리 상태(status) 필드가 없습니다** — "신고 대기"는 신고된 메시지가 아직 보이는(is_hidden=false) 상태를 뜻하며, 메시지를 숨기면 처리된 것으로 간주됩니다.
...
| 카테고리 삭제         | `/admin/boards/topics/[topic]/categories` | 카테고리의 게시글·댓글·추천       | ❌ (비활성화 권장) |
| 거래 등급 자동 생성   | `/admin/ranks/auto`                       | 기존 모든 거래 등급               | ❌                 |
| 게시판 등급 자동 생성 | `/admin/board-ranks/auto`                 | 기존 모든 게시판 등급 + 배지 할당 | ❌                 |
...
| QnA 카테고리 삭제     | `/admin/support`                          | 소속 QnA 전부                     | ❌ (비활성화 권장) |
| 홈 게시글 삭제        | `/admin/featured`                         | 게시글 + 첨부 이미지              | ❌                 |
...
| **권한 레벨 (숫자)**   | `auth_level` (Int, 기본 1)                   | 권한 레벨   | 사용자 화면(게시판·다운로드·작성·모더레이션)에서의 행동 가능 범위 |
...
## 부록 E — 두 가지 등급 시스템 (거래 등급 vs 게시판 등급)

| 구분      | 거래 등급                            | 게시판 등급                                |
| --------- | ------------------------------------ | ------------------------------------------ |
| 메뉴 위치 | 거래 게시판 > 거래 등급              | 게시판 > 게시판 등급                       |
| 경로      | `/admin/ranks`                       | `/admin/board-ranks`                       |
| 기준 값   | 사용자 **거래 횟수** (`trade_count`) | 사용자 **게시판 포인트** (`profile.point`) |
| 임계 필드 | 최소 거래 횟수 (`min_trade_count`)   | 최소 포인트 (`min_point`)                  |
| DB 테이블 | `rank` / `rank_badge`                | `board_rank` / `board_rank_badge`          |
| 자동 생성 | `/admin/ranks/auto`                  | `/admin/board-ranks/auto`                  |
| 배지 관리 | `/admin/ranks/badges`                | `/admin/board-ranks/badges`                |

> 두 시스템은 완전히 독립적이며 서로의 등급/배지/스냅샷에 영향을 주지 않습니다.

---

_이 문서는 HTML 매뉴얼 생성의 원본입니다. 페이지 신설/변경 시 본 문서를 먼저 업데이트한 뒤 HTML을 재생성하세요._
